...
dashboard poll-indicator fragment route. Thread-safe via a Lock.
"""
...
logger_placeholder = None  # no logger needed here
...
def get_all() -> list:
    ...
    Keys per dict: item_id, title, filename, pct (0-100), speed_str, bytes_done, total_bytes.
    """
    with _lock:
        result = []
...
def clear() -> None:
    """Remove all entries — called on startup to clear any stale state from a prior crash."""
    with _lock:
        _registry.clear()
