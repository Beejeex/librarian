"""File copy logic for MadTracked.
...
async def copy_file(
    ...
    Args:
        ...
        mode:    "copy" performs a chunked copy (hardlink kept for compat but not exposed in UI).
...
def get_file_size(path: str) -> int:
    ...
    Returns 0 if the file does not exist or cannot be read (e.g. race condition
    between copy completion and the size check).
    """
    try:
...
def check_quota(
    ...
    - New items can use the entire remaining capacity (total cap minus backlog usage),
      so unused backlog headroom is always available to new items.
    - A cap of 0 means unlimited.
    """
    if is_backlog:
        # Backlog pool: hard cap at 60% of total
        ...
        # New items: can use whatever the total cap has left (backlog + new combined)
...
def get_share_stats(share_path: str) -> dict:
    ...
    Used by the /api/share/stats endpoint for the dashboard widget.
    Returns a dict with keys: size_bytes (int), size_gb (float), file_count (int).
    """
    total_size = 0
    file_count = 0
    for root, _dirs, files in os.walk(share_path):
        for fname in files:
            try:
                ...
                pass  # File removed between walk and stat; skip it
    return {
